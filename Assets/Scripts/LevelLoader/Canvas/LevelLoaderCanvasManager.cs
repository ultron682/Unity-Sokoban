...
public class LevelLoaderCanvasManager : MonoBehaviour {
    ...
    void Start() {
        ...
        LevelDataManager.Instance.OnMovesCountChange += (movesCount) => { Text_MovesCount.text = "Ilość ruchów: " + movesCount.ToString(); };
        TileBoxContainerCounter.Instance.Refresh();

        LevelLoaderManager.Instance.OnEndLoadingLevel += (s, e) => {
            Panel_Summary.GetComponent<Panel_SummaryScript>().Text_Title.text = $"Ukończono poziom: {LevelDataManager.Instance.levelData_Serializable.id}";
        };
    }
